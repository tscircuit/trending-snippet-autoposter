...
export class TwitterPoster implements PlatformPoster {
  ...
  constructor() {
    ...
    if (!twitterToken) {
      console.error(
        "[TWITTER-POSTER] Missing Twitter token. Skipping Twitter posting.",
      );
      return;
    }
  ...
  async post(snippet: Snippet) {
    ...
    try {
      ...
      debug(
        `[TWITTER-POSTER] Successfully posted snippet to Twitter: ${snippet.name}`,
      );
      return true;
    } catch (error) {
      console.error(
        `[TWITTER-POSTER] Failed to post snippet to Twitter:`,
        error,
      );
    }
  }
